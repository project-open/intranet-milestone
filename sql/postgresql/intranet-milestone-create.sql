...
SELECT acs_permission__grant_permission(
	(select min(plugin_id) from im_component_plugins where plugin_name = 'Milestone Tracker'),
...
SELECT im_component_plugin__new (
	...
	null,					-- view_name
	120,					-- sort_order
	'im_milestone_list_component -project_id $project_id'
);

SELECT acs_permission__grant_permission(
	(select min(plugin_id) from im_component_plugins where plugin_name = 'Milestone Component'),
	(select group_id from groups where group_name = 'Employees'),
	'read'
);
